...
import {OnPhaseChanged} from "./on-phase-changed";
...
export class Timer {
  ...
  start(pomodoro: Pomodoro) {
    ...
    let sum=pomodoro.pauseDurationInSeconds+pomodoro.workDurationInSeconds;
    if (pomodoro.interrupted || difference>sum) {
    this.pause();
      this.secondsLeft = 0;
    } else if (difference > pomodoro.workDurationInSeconds) {
      this.started = true;
      this.secondsLeft = pomodoro.pauseDurationInSeconds - (difference - pomodoro.workDurationInSeconds);
      this.setPhase('PAUSE');
    } else {
      this.started = true;
      this.setPhase('WORK');
      this.secondsLeft = pomodoro.workDurationInSeconds - difference;
    }
    let minutes;
    let seconds;
    this.interval = setInterval(() => {
      if (this.secondsLeft > 0) {
        ...
        if (difference > pomodoro.workDurationInSeconds && this.phase !== 'PAUSE') {
          this.secondsLeft = pomodoro.pauseDurationInSeconds;
          this.setPhase('PAUSE');
        }
        this.secondsLeft--;
